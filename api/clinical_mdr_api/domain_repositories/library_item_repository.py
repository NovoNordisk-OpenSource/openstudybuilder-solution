...
class LibraryItemRepositoryImplBase(
    ...
    def exists_by(
        ...
        Args:
            ...
            on_root (bool, default False): A flag indicating whether to search on the root node. Defaults to False.
            exclude_archived (bool, default False): Exclude entities from Archived library
        ...
        exclusion_clause = (
            "any(label IN labels(root) WHERE NOT label STARTS WITH 'Deleted')"
        )
        if exclude_archived:
            exclusion_clause += f" AND NOT (:Library {{name: $excluded_library_name}})-[:{self.root_class.LIBRARY_REL_LABEL}]->(root)"

        if not on_root:
            query = f"""
                MATCH (root:{self.root_class.__label__})-[:LATEST]->(:{self.value_class.__label__} {{{property_name}: ${property_name}}})
            ...
            query = f"""
                MATCH (root:{self.root_class.__label__} {{{property_name}: ${property_name}}})-[:LATEST]->(:{self.value_class.__label__})
                WHERE {exclusion_clause}
                RETURN root
                """
        ...
        result, _ = db.cypher_query(
            ...
            {
                property_name: value,
                "excluded_library_name": settings.archived_library_name,
            },
        )
    ...
    def get_uid_by_property_value(self, property_name: str, value: str) -> str | None:
        query = f"""
            MATCH (root:{self.root_class.__label__})-[:LATEST_FINAL|LATEST_DRAFT|LATEST_RETIRED|LATEST]->(:{self.value_class.__label__} {{{property_name}: ${property_name}}})
    ...
    def find_uid_by_name(self, name: str) -> str | None:
        cypher_query = f"""
            MATCH (root:{self.root_class.__label__})-[:LATEST_FINAL|LATEST_DRAFT|LATEST_RETIRED]->(value:{self.value_class.__label__} {{name: $name }})
            WHERE NOT (:Library {{name: $excluded_library_name}})-[:{self.root_class.LIBRARY_REL_LABEL}]->(root)
            RETURN root.uid
        """
    ...
    def get_property_by_uid(self, uid: str, prop: str) -> str | None:
        cypher_query = f"""
            MATCH (root:{self.root_class.__label__} {{uid: $uid }})-[:LATEST_FINAL|LATEST_DRAFT|LATEST_RETIRED]->(value:{self.value_class.__label__})
            RETURN value.{prop}
        """
    ...
    # Bypass the cache on for_update=True so that _lock_object() always runs
    # and a Neo4j write lock is held in the current transaction. Neo4j locks
    # are transaction-scoped, so a cached AR from a prior (committed)
    # transaction would otherwise be returned without any lock being held.
    @cached_skip_on(
        "for_update",
    ...
    def find_by_uid_minimal(
        ...
        *,
        ...
        """
        Retrieve a single item by UID with only requested fields.
        Returns a dictionary instead of a full aggregate root.

        This is the single-entity equivalent of find_all_minimal().
        """
        # Build match clause with uid filter
        match_clause = self.generic_match_clause(version=version)
        match_clause += "\nWHERE odm_root.uid = $uid"

        # Build minimal alias clause (reuse existing infrastructure)
        alias_clause = self._build_minimal_alias_clause(select_fields, version=version)

        # Add status/date filtering in WHERE clause
        additional_filters = []
        if status is not None:
            additional_filters.append("version_rel.status = $status")
        if at_specific_date is not None:
            additional_filters.append("version_rel.start_date <= $at_specific_date")
            additional_filters.append(
                "(version_rel.end_date IS NULL OR version_rel.end_date > $at_specific_date)"
            )

        if additional_filters:
            # Need to add WHERE after the WITH clause
            alias_clause += "\nWHERE " + " AND ".join(additional_filters)

        # Build and execute query
        query_params: dict[str, Any] = {"uid": uid}
        if version is not None:
            query_params["requested_version"] = version
        if status is not None:
            query_params["status"] = status.value
        if at_specific_date is not None:
            query_params["at_specific_date"] = at_specific_date

        query_string = f"{match_clause}\nWITH {alias_clause}\nRETURN *"

        result_array, attribute_names = db.cypher_query(query_string, query_params)

        if len(result_array) == 0:
            return None

        # Extract single item (same as find_all_minimal)
        item_dict = {}
        for item_property, attribute_name in zip(result_array[0], attribute_names):
            item_dict[attribute_name] = self._convert_neo4j_types(item_property)

        return item_dict

    def _convert_neo4j_types(self, value: Any) -> Any:
        """
        Recursively convert Neo4j types to Python native types for JSON serialization.
        ...
        if isinstance(value, dict):
            return {key: self._convert_neo4j_types(val) for key, val in value.items()}
        return value
    ...
    # Bypass the cache on for_update=True so that _lock_object() always runs
    # and a Neo4j write lock is held in the current transaction.
    @cached_skip_on(
        "for_update",
        cache=cache_store_item_by_uid,
...
""")
    ...
    def _ctterm_name_match_return_stmt(self):
        match_stmt = """
            ...
            WITH *, ctcodelist_root.uid IN $ts_codelist_uids AS is_ts_term
            CALL apoc.when(
                coalesce(is_ts_term, false),
                'OPTIONAL MATCH (value)-[:RELATED_STUDY_FIELD_SELECTION]->(msf:MetaStudyField)
                 ...
                 OPTIONAL MATCH (value)-[:RESPONSE_CODELIST]->(resp_cl:CTCodelistRoot)
                 OPTIONAL MATCH (resp_cl)-[:HAS_NAME_ROOT]->(:CTCodelistNameRoot)-[:LATEST_FINAL]->(resp_cl_name:CTCodelistNameValue)
                 OPTIONAL MATCH (value)-[:RESPONSE_DICTIONARY]->(resp_dict:DictionaryCodelistRoot)
                 OPTIONAL MATCH (value)-[:VALID_NULL_FLAVOR]->(:CTTermContext)-[:HAS_SELECTED_TERM]->(nf_term:CTTermRoot)
                 OPTIONAL MATCH (nf_term)-[:HAS_NAME_ROOT]->(:CTTermNameRoot)-[:LATEST_FINAL]->(nf_name:CTTermNameValue)
                 RETURN msf.osb_field_name AS osb_field_name, msf.osb_page_reference AS osb_page_reference, dt_term.uid AS semantic_data_type_uid, dt_name.name AS semantic_data_type_name,
                        resp_cl.uid AS response_codelist_uid,
                        resp_cl_name.name AS response_codelist_name,
                        collect(DISTINCT resp_dict.uid) AS response_dictionary_uids,
                        collect(DISTINCT nf_term.uid) AS valid_null_flavor_term_uids,
                        collect(DISTINCT nf_name.name) AS valid_null_flavor_term_names',
                'RETURN null AS osb_field_name, null AS osb_page_reference, null AS semantic_data_type_uid, null AS semantic_data_type_name,
                        null AS response_codelist_uid,
                        null AS response_codelist_name,
                        [] AS response_dictionary_uids,
                        [] AS valid_null_flavor_term_uids,
                        [] AS valid_null_flavor_term_names',
                {value: value}
            ) YIELD value AS ts_props
        """
        ctterm_names_return = """,
            {
                ctterm_root_uid: ctterm_root.uid,
                ctterm_name_element_id: elementID(root),
                osb_field_name: ts_props.osb_field_name,
                osb_page_reference: ts_props.osb_page_reference,
                semantic_data_type_uid: ts_props.semantic_data_type_uid,
                semantic_data_type_name: ts_props.semantic_data_type_name,
                response_codelist_uid: ts_props.response_codelist_uid,
                response_codelist_name: ts_props.response_codelist_name,
                response_dictionary_uids: ts_props.response_dictionary_uids,
                valid_null_flavor_term_uids: ts_props.valid_null_flavor_term_uids,
                valid_null_flavor_term_names: ts_props.valid_null_flavor_term_names
            } as ctterm_name
        """
    ...
    def _activity_instance_root_match_return_stmt(self):
        match_stmt = """
            ...
            CALL(root, ver_rel, activity_instance_value) {
