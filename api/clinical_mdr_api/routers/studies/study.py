...
@router.post(
    "/studies/{study_uid}/study-arms/copy-from-study/{study_source_uid}",
    dependencies=[security, rbac.STUDY_WRITE],
    summary="Copy all study arms from another study into the target study",
    description="""
State before:
 - Target study (`study_uid`) must exist and not be locked.
 - Source study (`study_source_uid`) must exist and differ from the target.

Authorization:
 - Requires write access to the target study (same in-tenant model as copy-component).
 - Source study metadata is read when the source UID is provided; no separate
   source-study permission is enforced beyond existence checks.

State after:
 - All source study arms are created on the target using batch conflict resolution
   (skip, suffix, or adjust fields when duplicates are detected).
""",
    status_code=207,
    responses={
        403: _generic_descriptions.ERROR_403,
...
@decorators.validate_if_study_is_not_locked("study_uid")
def study_arms_copy_from_study(
    study_uid: Annotated[str, studyUID],
    study_source_uid: Annotated[str, studyUID],
) -> list[StudySelectionArmBatchOutput]:
    service = StudyArmSelectionService()
    return service.copy_arms_from_study(
        study_uid=study_uid, study_source_uid=study_source_uid
    )
...
@router.post(
    "/studies/{study_uid}/study-elements/batch",
    dependencies=[security, rbac.STUDY_WRITE],
    summary="Batch create and/or edit of study elements",
    status_code=207,
    responses={
        403: _generic_descriptions.ERROR_403,
        404: _generic_descriptions.ERROR_404,
...
@decorators.validate_if_study_is_not_locked("study_uid")
def study_elements_batch_operations(
    study_uid: Annotated[str, studyUID],
    operations: Annotated[list[StudySelectionElementBatchInput], Body()],
...
@router.post(
    "/studies/{study_uid}/study-elements/copy-from-study/{study_source_uid}",
    dependencies=[security, rbac.STUDY_WRITE],
    summary="Copy all study elements from another study into the target study",
    description="""
State before:
 - Target study (`study_uid`) must exist and not be locked.
 - Source study (`study_source_uid`) must exist and differ from the target.
...
State after:
 - All source study elements are created on the target using skip deduplication
   when name/sub type or short name/sub type duplicates are detected.
""",
    status_code=207,
    responses={
        403: _generic_descriptions.ERROR_403,
...
@decorators.validate_if_study_is_not_locked("study_uid")
def study_elements_copy_from_study(
    study_uid: Annotated[str, studyUID],
    study_source_uid: Annotated[str, studyUID],
) -> list[StudySelectionElementBatchOutput]:
    service = StudyElementSelectionService()
    return service.copy_elements_from_study(
        study_uid=study_uid, study_source_uid=study_source_uid
    )
...
@router.post(
    "/studies/{study_uid}/study-branch-arms/copy-from-study/{study_source_uid}",
    dependencies=[security, rbac.STUDY_WRITE],
    summary="Copy all study branch arms from another study into the target study",
    description="""
State before:
 - Target study (`study_uid`) must exist and not be locked.
 - Source study (`study_source_uid`) must exist and differ from the target.
...
State after:
 - All source study branch arms are created on the target using batch conflict resolution
   (skip when duplicates are detected; parent study arms resolved by name and arm type).
""",
    status_code=207,
    responses={
        403: _generic_descriptions.ERROR_403,
...
@decorators.validate_if_study_is_not_locked("study_uid")
def study_branch_arms_copy_from_study(
    study_uid: Annotated[str, studyUID],
    study_source_uid: Annotated[str, studyUID],
) -> list[StudySelectionBranchArmBatchOutput]:
    service = StudyBranchArmSelectionService()
    return service.copy_branch_arms_from_study(
        study_uid=study_uid, study_source_uid=study_source_uid
    )
