...
@decorators.allow_exports(
    {
        ...
        # Lag time is legacy-only, so it is exported on request rather than by
        # default.
        "variants": {
            "with_lag_time": {
                "defaults": [
...
@router.post(
    "/studies/{study_uid}/study-epochs/batch/preview",
    dependencies=[security, rbac.STUDY_WRITE],
    summary="Preview a batch of study epochs",
    response_model_exclude_unset=True,
    status_code=200,
    responses={
        403: _generic_descriptions.ERROR_403,
...
@decorators.validate_if_study_is_not_locked("study_uid")
def post_preview_epochs_batch(
    study_uid: Annotated[str, studyUID],
    selections: Annotated[
        list[study_epoch.StudyEpochCreateInput],
        Body(description="Study epochs to preview."),
    ],
) -> study_epoch.StudyEpochBatchPreviewResponse:
    service = StudyEpochService(study_uid=study_uid)
    return service.preview_batch_operations(study_uid=study_uid, specs=selections)


@router.post(
    "/studies/{study_uid}/study-epochs/batch",
    dependencies=[security, rbac.STUDY_WRITE],
    summary="Batch create and/or edit of study epochs",
    status_code=207,
    responses={
        403: _generic_descriptions.ERROR_403,
        404: _generic_descriptions.ERROR_404,
...
@decorators.validate_if_study_is_not_locked("study_uid")
def study_epochs_batch_operations(
    study_uid: Annotated[str, studyUID],
    operations: Annotated[list[study_epoch.StudyEpochBatchInput], Body()],
) -> study_epoch.StudyEpochBatchResponse:
    service = StudyEpochService(study_uid=study_uid)
