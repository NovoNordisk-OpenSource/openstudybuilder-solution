    ...
    with trace_block("generic_item_filtering") as span:
        ...
        if filter_operator == FilterOperator.AND:
            ...
            if not filters.elements:
                ...
                # Finally, deduplicate list by object identity. Using `id(item)`
                # instead of `item.uid` because not all item models expose a
                # `uid` attribute (e.g. StudySelectionActivity uses
                # `study_activity_uid`). Same-object references are what we
                # need to dedupe regardless of the model's field naming.
                seen: set[int] = set()
                filtered_items = []
