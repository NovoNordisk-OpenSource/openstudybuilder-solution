...
from clinical_mdr_api.domains.odms.utils import EN_LANGUAGE, TargetType
from clinical_mdr_api.domains.odms.xml_definition import (
    ODM,
    Alias,
    Attribute,
    BasicDefinitions,
...
from clinical_mdr_api.models.odms.common_models import OdmRefVendorAttributeModel
...
class OdmXmlExporterService:
    ...
    def __init__(
        ...
        Args:
            ...
            pdf (bool): A flag indicating whether to generate a PDF.
    ...
    def get_odm_document(self) -> OdmDataExtractor | bytes:
        ...
        This method generates an ODM XML document from the internal ODM representation. Depending on
        the export format specified, it can return the data in different formats:
        - HTML: Returns the OdmDataExtractor object directly
        - XML: Returns a pretty-printed XML byte string
        - PDF: Returns a PDF byte string generated by applying an XSL stylesheet transformation

        Returns:
            OdmDataExtractor | bytes:
                - Pretty-printed XML as bytes if export_type is XML
                - PDF document as bytes if export_type is PDF

        Raises:
            BusinessLogicException:
                - If stylesheet is not provided when PDF generation is requested
                - If an error occurs during PDF generation (wraps the underlying exception)
        """
    ...
    def return_if_vendor_element_is_allowed(self, element) -> Element | None:
        """
        Returns the given ODM element if its namespace is allowed by `self.allowed_namespaces`.
        """

        if not element._custom_element_name:
            return None

        if "*" in self.allowed_namespaces:
            return element

        prefix, _ = element._custom_element_name.split(":")
        if prefix in self.allowed_namespaces:
            return element
    ...
    def _create_odm_object(self):
                            ...
                            TranslatedText(
                                ...
                                lang=Attribute(
                                    ...
                                    get_iso_lang_data(  # type: ignore[arg-type]
                                        query=translated_text.language or EN_LANGUAGE
                                    ),
                                ),
                            )
                            for translated_text in form.translated_texts
                            if translated_text.text_type
                            == OdmTranslatedTextTypeEnum.DESCRIPTION
                        ]
                    ),
                    osb_design_notes=self.return_if_vendor_element_is_allowed(
                        OsbDesignNotes(
                            [
                                TranslatedText(
                                    translated_text.text,
                                    lang=Attribute(
                                        self.XML_LANG,
                                        get_iso_lang_data(  # type: ignore[arg-type]
                                            query=translated_text.language
                                            or EN_LANGUAGE,
                                        ),
                                    ),
                                )
                                for translated_text in form.translated_texts
                                if translated_text.text_type
                                == OdmTranslatedTextTypeEnum.OSB_DESIGN_NOTES
                            ]
                        )
                    ),
                    osb_completion_instructions=self.return_if_vendor_element_is_allowed(
                        OsbCompletionInstructions(
                            [
                                TranslatedText(
                                    translated_text.text,
                                ...
                                for translated_text in form.translated_texts
                                if self.OSB_NAMESPACE in self.allowed_namespaces
                                and translated_text.text_type
                                == OdmTranslatedTextTypeEnum.OSB_COMPLETION_INSTRUCTIONS
                            ]
                        )
                    ),
                    osb_display_text=self.return_if_vendor_element_is_allowed(
                        OsbDisplayText(
                            [
                                TranslatedText(
                                    translated_text.text,
                                ...
                                if self.OSB_NAMESPACE in self.allowed_namespaces
                                and translated_text.text_type
                                == OdmTranslatedTextTypeEnum.OSB_DISPLAY_TEXT
                            ]
                        )
                    ),
                    aliases=[
                    ...
                    **self._create_vendor_elements_of(item_group),
                    ...
                    description=Description(
                        [
                            ...
                            for translated_text in item_group.translated_texts
                            if translated_text.text_type
                            == OdmTranslatedTextTypeEnum.DESCRIPTION
                        ]
                    ),
        ...
        def create_odm_item_def():
            ...
            for item in self.odm_data_extractor.odm_items:
                activity_instances = []
                _seen_activity_instances: set[tuple[str, Any, Any]] = set()

                activity_instances_by_uid: dict[str, list[Any]] = {}

                for activity_instance in item.activity_instances:
                    activity_instances_by_uid.setdefault(
                        activity_instance.activity_instance_uid, []
                    ).append(activity_instance)

                for _activity_instances in activity_instances_by_uid.values():
                    _key = (
                        _activity_instances[0].activity_instance_name or "",
                        _activity_instances[0].activity_instance_adam_param_code,
                        _activity_instances[0].activity_instance_topic_code,
                    )
                    if _key in _seen_activity_instances:
                        continue
                    _seen_activity_instances.add(_key)

                    activity_item_classes = [
                        act.activity_item_class_name for act in _activity_instances
                    ]

                    activity_instances.append(
                        OsbActivityInstance(
                            _string=_activity_instances[0].activity_instance_name or "",
                            adam_code=Attribute(
                                "osb:adamCode",
                                _activity_instances[
                                    0
                            ...
                            activity_item_classes=Attribute(
                                "osb:activityItemClasses",
                                ", ".join(activity_item_classes),
                            ),
                            is_derived=Attribute("osb:isDerived", "true"),
                        )
                    )
                item_defs.append(
                    ItemDef(
                        ...
                        datatype=Attribute(
                            "DataType",
                            item.datatype.submission_value if item.datatype else None,
                        ),
                        ...
                        **self._create_vendor_elements_of(item),
                        ...
                        description=Description(
                            [
                                ...
                                for translated_text in item.translated_texts
                                if translated_text.text_type
                                == OdmTranslatedTextTypeEnum.DESCRIPTION
                            ]
                        ),
                        question=Question(
                            [
                                ...
                                for translated_text in item.translated_texts
                                if translated_text.text_type
                                == OdmTranslatedTextTypeEnum.QUESTION
                            ]
                        ),
                        external_question=ExternalQuestion(
                            Attribute(
                                "Dictionary",
                                (
                                    item.external_question.codelist_uid
                                    if item.external_question
                                    else None
                                ),
                            ),
                            Attribute(
                            ...
                            Attribute(
                                "Version",
                                (
                                    item.external_question.version
                                    if item.external_question
                                    else None
                                ),
                            ),
                        ...
                        activity_instances=(
                            self._get_vendor_elements_or_empty_list(activity_instances)
                        ),
                        codelist_ref=CodeListRef(
                            ...
                            **self._get_vendor_attributes_or_empty_dict(
                                {
                                    "osb_allows_multi_choice": Attribute(
                                        "osb:allowsMultiChoice",
                                        getattr(
                                            item.codelist, "allows_multi_choice", False
                                        ),
                                    )
                                }
                            ),
                    ...
                    **self._get_vendor_attributes_or_empty_dict(
                    ...
                    description=Description(
                        [
                            ...
                            for translated_text in condition.translated_texts
                            if translated_text.text_type
                            == OdmTranslatedTextTypeEnum.DESCRIPTION
                        ]
                    ),
                    ...
                    **self._get_vendor_attributes_or_empty_dict(
                    ...
                    description=Description(
                        [
                            ...
                            for translated_text in method.translated_texts
                            if translated_text.text_type
                            == OdmTranslatedTextTypeEnum.DESCRIPTION
                        ]
                    ),
