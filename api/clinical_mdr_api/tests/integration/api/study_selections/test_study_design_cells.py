...
def test_clone_study_async_creates_retrievable_background_job(api_client):
    response = api_client.post(
        f"/studies/{study.uid}/clone/async",
        json={
            "study_number": "6678",
            "study_acronym": "6678",
            "project_number": PROJECT_NUMBER,
            "description": "Async clone",
            "copy_study_arm": True,
            "copy_study_branch_arm": False,
            "copy_study_cohort": False,
            "copy_study_element": False,
    ...
    assert_response_status_code(response, 200)
    listed_job = next(job for job in response.json()["items"] if job["uid"] == job_uid)
    assert listed_job["job_name"].startswith("Clone study ")
    assert listed_job["job_name"].endswith(" to 123-6678")
    assert listed_job["response"] is None

    deadline = time.monotonic() + 60
    while True:
        response = api_client.get(f"/jobs/{job_uid}")
        assert_response_status_code(response, 200)
        completed_job = response.json()
        if completed_job["status"] != "RUNNING":
            break
        assert time.monotonic() < deadline, "Background job did not finish in time"
        time.sleep(0.1)

    assert completed_job["status"] == "COMPLETED"
    assert completed_job["response"]["uid"]
    assert completed_job["log"]
