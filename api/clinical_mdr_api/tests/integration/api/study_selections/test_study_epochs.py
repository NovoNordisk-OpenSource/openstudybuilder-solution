...
def test_study_epoch_lag_time(api_client):
    """Lag time only carries data migrated from MMA."""
    lag_study = TestUtils.create_study()
    # the base data seeds days; weeks does not exist in the test fixtures, so
    # create it here; lag time accepts only the plural forms
    days_uid = TestUtils.get_unit_uid_by_name("days")
    weeks_uid = TestUtils.create_unit_definition(name="weeks").uid

    response = api_client.post(
        f"/studies/{lag_study.uid}/study-epochs",
        json={
            "study_uid": lag_study.uid,
            "epoch_subtype": epoch_subtype_uid,
            "ae_lag_time": 5,
            "ae_lag_time_unit_uid": days_uid,
            "hypo_lag_time": 2,
            "hypo_lag_time_unit_uid": weeks_uid,
        },
    )
    res = response.json()
    assert_response_status_code(response, 201)
    assert res["ae_lag_time"] == 5
    assert res["ae_lag_time_unit_uid"] == days_uid
    assert res["ae_lag_time_unit_name"] == "days"
    assert res["hypo_lag_time"] == 2
    assert res["hypo_lag_time_unit_name"] == "weeks"
    assert res["ce_lag_time"] is None
    assert res["ce_lag_time_unit_uid"] is None
    lag_epoch_uid = res["uid"]

    # editing must reconnect the unit rather than keep the one copied from the previous version
    response = api_client.patch(
        f"/studies/{lag_study.uid}/study-epochs/{lag_epoch_uid}",
        json={
            "study_uid": lag_study.uid,
            "ae_lag_time": 3,
            "ae_lag_time_unit_uid": weeks_uid,
            "change_description": "new lag time",
        },
    )
    res = response.json()
    assert_response_status_code(response, 200)
    assert res["ae_lag_time"] == 3
    assert res["ae_lag_time_unit_uid"] == weeks_uid
    assert res["ae_lag_time_unit_name"] == "weeks"
    # omitted fields keep their value, like every other field on this endpoint
    assert res["hypo_lag_time"] == 2
    assert res["hypo_lag_time_unit_name"] == "weeks"
    ...
    # an explicit null clears the lag time and disconnects its unit
    response = api_client.patch(
        f"/studies/{lag_study.uid}/study-epochs/{lag_epoch_uid}",
        json={
            "study_uid": lag_study.uid,
            "hypo_lag_time": None,
            "hypo_lag_time_unit_uid": None,
            "change_description": "clear hypo lag time",
        },
    )
    res = response.json()
    assert_response_status_code(response, 200)
    assert res["hypo_lag_time"] is None
    assert res["hypo_lag_time_unit_uid"] is None
    assert res["hypo_lag_time_unit_name"] is None
    assert res["ae_lag_time"] == 3

    # the audit trail is built from a separate code path, so check lag time survives it
    response = api_client.get(
        f"/studies/{lag_study.uid}/study-epochs/{lag_epoch_uid}/audit-trail",
    )
    res = response.json()
    assert_response_status_code(response, 200)
    assert [version["ae_lag_time"] for version in res] == [3, 3, 5]
    assert [version["hypo_lag_time"] for version in res] == [None, 2, 2]
    assert [version["hypo_lag_time_unit_name"] for version in res] == [
        None,
        "weeks",
        "weeks",
    ]

    # clearing only the value must drop the unit too, otherwise the partial-payload
    # merge copies the old unit uid back in and leaves a unit with nothing to qualify
    response = api_client.patch(
        f"/studies/{lag_study.uid}/study-epochs/{lag_epoch_uid}",
        json={
            "study_uid": lag_study.uid,
            "ae_lag_time": None,
            "change_description": "clear ae lag time value only",
        },
    )
    res = response.json()
    assert_response_status_code(response, 200)
    assert res["ae_lag_time"] is None
    assert res["ae_lag_time_unit_uid"] is None
    assert res["ae_lag_time_unit_name"] is None


def test_study_epoch_lag_time_rejects_invalid_input(api_client):
    """Lag time only carries data migrated from MMA."""
    lag_study = TestUtils.create_study()
    # the singular day exists for study visits and is exactly what lag time must reject;
    # days is seeded by base data and is the valid unit the patches further down use
    day_uid = TestUtils.get_unit_uid_by_name("day")
    days_uid = TestUtils.get_unit_uid_by_name("days")

    # a unit uid that is not a unit definition at all
    response = api_client.post(
        f"/studies/{lag_study.uid}/study-epochs",
        json={
            "study_uid": lag_study.uid,
            "epoch_subtype": epoch_subtype_uid,
            "ae_lag_time": 5,
            "ae_lag_time_unit_uid": "UnitDefinition_000000",
        },
    )
    assert_response_status_code(response, 400)
    assert "does not exist" in response.json()["message"]

    # lag time is a duration, so it cannot be negative
    response = api_client.post(
        f"/studies/{lag_study.uid}/study-epochs",
        json={
            "study_uid": lag_study.uid,
            "epoch_subtype": epoch_subtype_uid,
            "ae_lag_time": -5,
            "ae_lag_time_unit_uid": day_uid,
        },
    )
    assert_response_status_code(response, 400)
    assert response.json()["details"][0]["field"] == ["body", "ae_lag_time"]

    # lag time came from MMA, which recorded days and weeks; the singular day and
    # week stay valid for study visits but are not a lag time
    response = api_client.post(
        f"/studies/{lag_study.uid}/study-epochs",
        json={
            "study_uid": lag_study.uid,
    ...
    assert_response_status_code(response, 400)
    assert "must be one of" in response.json()["message"]

    # the same checks apply on edit, not just on create
    response = api_client.post(
        f"/studies/{lag_study.uid}/study-epochs",
        json={"study_uid": lag_study.uid, "epoch_subtype": epoch_subtype_uid},
    )
    assert_response_status_code(response, 201)
    lag_epoch_uid = response.json()["uid"]
    ...
    response = api_client.patch(
        ...
        json={
            ...
            "ae_lag_time": 1,
            "ae_lag_time_unit_uid": day_uid,
            "change_description": "unit outside the allowed set",
        },
    )
    assert_response_status_code(response, 400)
    assert "must be one of" in response.json()["message"]

    # a value with no unit is not a duration
    response = api_client.post(
        f"/studies/{lag_study.uid}/study-epochs",
        json={
            "study_uid": lag_study.uid,
    ...
    assert_response_status_code(response, 400)
    assert "requires a lag time unit" in response.json()["message"]
    ...
    response = api_client.patch(
        ...
        json={
            "study_uid": lag_study.uid,
            "ae_lag_time": 5,
            "change_description": "value with no unit",
        },
    )
    assert_response_status_code(response, 400)
    assert "requires a lag time unit" in response.json()["message"]

    # but once a unit is stored, patching the value alone is a complete duration
    response = api_client.patch(
        f"/studies/{lag_study.uid}/study-epochs/{lag_epoch_uid}",
        json={
            "study_uid": lag_study.uid,
            "ae_lag_time": 5,
            "ae_lag_time_unit_uid": days_uid,
            "change_description": "set lag time",
        },
    )
    assert_response_status_code(response, 200)
    ...
    response = api_client.patch(
        ...
        json={
            "study_uid": lag_study.uid,
            "ae_lag_time": 7,
            "change_description": "change value only",
        },
    )
    assert_response_status_code(response, 200)
    assert response.json()["ae_lag_time"] == 7
    assert response.json()["ae_lag_time_unit_uid"] == days_uid
