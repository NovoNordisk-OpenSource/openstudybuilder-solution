...
class TestStudyEpochManagement(unittest.TestCase):
    ...
    def test__create_study_epoch_with_explicit_order__existing_epochs_shifted_without_gap_or_duplicate(
        self,
    ):
        epoch_service = StudyEpochService()
        epoch_subtype_uid = "EpochSubType_0001"
        ...
        insert_input = StudyEpochCreateInput(
            study_uid="study_root",
            start_rule="start_rule",
            end_rule="end_rule",
            description="inserted-at-order-2",
            epoch_subtype=epoch_subtype_uid,
            duration=0,
            color_hash="#1100FF",
            order=2,
        )
        epoch_service.create(study_uid=self.study.uid, study_epoch_input=insert_input)

        epochs = sorted(
            epoch_service.get_all_epochs(self.study.uid).items, key=lambda e: e.order
        )
        orders = [e.order for e in epochs]
        self.assertEqual(orders, [1, 2, 3, 4])
        self.assertEqual(len(orders), len(set(orders)))
        self.assertEqual(epochs[1].description, "inserted-at-order-2")
