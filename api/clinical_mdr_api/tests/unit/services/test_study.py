...
@dataclass(frozen=True)
class RegistryIdentifiersForLookupTest:
...
class TestStudyService(unittest.TestCase):
    ...
    def test__determine_needed_sections__applies_excludes_after_includes(self):
        result = StudyService._determine_needed_sections(
    ...
    def test__build_batched_term_lookup__keeps_cache_per_specific_date(self):
        term_uid = "TERM_001"
        prefetched_date = datetime(2025, 1, 1, tzinfo=timezone.utc)
        prefetched_term = create_random_ct_term_name_ar(
            generate_uid_callback=lambda: term_uid
        )
        undated_term = create_random_ct_term_name_ar(
            generate_uid_callback=lambda: term_uid
        )

        study_service = StudyService.__new__(StudyService)
        study_service._repos = MagicMock()
        study_service._repos.study_definition_repository.find_by_uid.return_value = None
        study_service._repos.ct_term_name_repository.find_by_uids.side_effect = [
            [prefetched_term],
            [undated_term],
        ]

        lookup = study_service._build_batched_term_lookup(
            study_definition=SimpleNamespace(
                draft_metadata=None,
                released_metadata=None,
                latest_locked_metadata=None,
                version_specific_metadata=None,
                study_parent_part_uid=None,
                current_metadata=SimpleNamespace(
                    id_metadata=SimpleNamespace(
                        registry_identifiers=RegistryIdentifiersForLookupTest(
                            eudract_id_null_value_code=term_uid
                        )
                    ),
            ...
            terms_at_specific_datetime=prefetched_date,
            needed_sections={StudyComponentEnum.IDENTIFICATION_METADATA.value},
        )

        self.assertEqual(
            lookup([term_uid], at_specific_date=prefetched_date),
            [prefetched_term],
        )
        self.assertEqual(
            study_service._repos.ct_term_name_repository.find_by_uids.call_count,
            1,
        )

        self.assertEqual(lookup([term_uid], at_specific_date=None), [undated_term])
        self.assertEqual(
            study_service._repos.ct_term_name_repository.find_by_uids.call_count,
            2,
        )
