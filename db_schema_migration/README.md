...
> These scripts are pinned to a specific migration number in `Pipfile`
> (`migration_024` and `correction_022` at the time of writing). Bump those
> entries when starting a new migration.
...
```
DATABASE_URL=bolt://neo4j:<your-password>@localhost:7687
...
as the test script. 
  ...
  ```
  NEO4J_DSN=bolt://neo4j:<your-password>@localhost:7687
  NEO4J_DATABASE=schema.migration.test
  ```

# Data corrections

This repository also contains data corrections, that correct errors and mistakes in the OpenStudyBuilder database.
...
Create a file in `tests/data` called `db_before_migration_{number}.py`
