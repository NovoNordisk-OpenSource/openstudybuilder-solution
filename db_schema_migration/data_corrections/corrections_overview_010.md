...
List of relevant items used for populating `Activity Instance` template parameters is too wide, as described in ADO bug #2621134 (Digital Data Streams ART, archived).
...
End-user QPHM has performed changes on some Library and Study entities in 2024. However, he doesn't exist in MS Graph anymore, so db schema migration [script](../migrations/migration_010.py) was not able to create `User` node representing him.
