...
class SchemaMigrationNode(contextlib.AbstractContextManager):
    """Context manager that creates a SchemaMigration node on enter and sets finished result on exiting."""

    def __init__(self, filename: str, driver: neo4j.Neo4jDriver):
        self.name = os.path.splitext(os.path.basename(filename))[0]
        self.driver = driver
        self.uuid = str(uuid.uuid4())

    def __enter__(self):
        logger.info("Creating SchemaMigration node '%s' [%s]", self.name, self.uuid)
        with self.driver.session(database=DATABASE_NAME) as sess:
            sess.run(
                "CREATE (m:SchemaMigration {uuid: $uuid, name: $name, started: datetime()})",
                uuid=self.uuid,
                name=self.name,
            )
        return self  # optional: caller can inspect .uuid or .name

    def __exit__(self, exc_type, exc_value, traceback):
        result_str = "OK" if exc_value is None else "FAILED"
        logger.info(
            "Updating SchemaMigration node [%s] with result: %s", self.uuid, result_str
        )
        with self.driver.session(database=DATABASE_NAME) as sess:
            # set finished timestamp and result
            sess.run(
                "MATCH (m:SchemaMigration {uuid: $uuid}) SET m.finished = datetime(), m.result = $result",
                uuid=self.uuid,
                result=result_str,
            )
