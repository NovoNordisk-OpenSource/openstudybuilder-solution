...
def upload_attachment(json_name):
    ...
    wiki_url = f"https://dev.example.com/{project}/_apis/wiki/wikis/{wiki_identifier}/attachments?name={name}&api-version=7.0"
...
def make_page_url(path):
    ...
    return f"https://dev.example.com/{project}/_apis/wiki/wikis/{wiki_identifier}/pages?path={encoded_path}&api-version=7.0"
...
def main():
    ...
    create_page_with_full_path(
        new_md, f"{WIKI_PATH}/{WIKI_PAGE_NAME} {formatted_datetime}"
    )
