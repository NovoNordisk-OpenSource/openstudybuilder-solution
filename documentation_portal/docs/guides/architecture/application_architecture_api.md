...
[![responsibility segregation overview sequence diagram](/images/documentation/ddd_responsibility_segregation_overview.png)](/images/documentation/ddd_responsibility_segregation_overview.png)
...
* classes used as API exceptions in `common/exceptions.py`, shared by all three APIs and imported as `from common.exceptions import ...`

**Repositories** code (those which follow DDD pattern) is located in `clinical_mdr_api/domain_repositories`. 
Furthermore, a general convention is assumed to place the code defining and implementing a repository for given aggregate inside subpackage named after the aggregate (e.g. `clinical_mdr_api/domain_repositories/study_definitions`).

**Domain layer** code is located in `clinical_mdr_api/domains` package with a general convention of creating a separate subpackage for code implementing particular aggregate (e.g. `clinical_mdr_api/domains/study_definition_aggregates` for study definition aggregate). However, same as for repositories the latter may not be strictly followed in some special cases.
...
[![dependency-diagram](/images/documentation/dependency-graph.png)](/images/documentation/dependency-graph.png)
