...
export default {
  ...
  /**
   * Studies for copy-from-study selectors (StudyMinimal fields).
   * Uses API default minimal_response=true; sufficient for StudySelectorField
   * and keeps has_study_* filters without the heavier non-minimal payload.
   */
  getSelectableStudies(options = {}) {
    return this.getIds(options)
  },
  projects_all() {
    const params = {
      page_size: 0,
  ...
  cloneStudy(studyUid, data) {
    return repository.post(`${resource}/${studyUid}/clone/async`, data)
  },
  ...
  getOtherAttributes(studyUid, params) {
    return repository.get(`${resource}/${studyUid}/study-other-attributes`, {
      params,
    })
  },
  batchUpdateOtherAttributes(studyUid, data) {
    return repository.post(
      `${resource}/${studyUid}/study-other-attributes/batch`,
      data
    )
  },
}
