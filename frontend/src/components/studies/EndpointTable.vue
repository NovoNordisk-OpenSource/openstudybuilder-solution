<template>
  <NNTable
    ref="table"
    key="endpointTable1"
    table-id="studies-endpoints-table"
    :headers="headers"
    :items="formattedStudyEndpoints"
    ...
    :hide-default-body="sortMode && formattedStudyEndpoints.length > 0"
    :history-config="{
      dataFetcher: fetchEndpointsHistory,
      htmlFields: historyHtmlFields,
      title: $t('StudyEndpointsTable.global_history_title'),
    }"
    :export-config="{ objectLabel: 'StudyEndpoints', dataUrl: exportDataUrl }"
  ...
  >
    ...
    <template #tbody>
      <tbody v-show="sortMode" ref="parent">
        <tr
          v-for="endpoint in formattedStudyEndpoints"
...
const [parent, formattedStudyEndpoints] = useDragAndDrop([], {
  onDragend: (event) => {
    const newOrder =
...
function fetchEndpoints(filters, options, filtersUpdated) {
  ...
  studiesEndpointsStore.fetchStudyEndpoints(params).then((resp) => {
    formattedStudyEndpoints.value = transformItems(resp.data.items)
    loading.value = false
  })
}
