    ...
    <template #[`step.selectFromStudies`]>
      <v-col cols="12">
        ...
        <NNTable
          ...
          table-id="study-activities-dialog-study-table"
          item-value="uid"
          show-select
    ...
    <template #[`step.selectFromLibrary`]>
      <v-col cols="12">
        ...
        <NNTable
          ...
          v-model="selectedActivities"
          table-id="study-activities-dialog-library-table"
          item-value="item_key"
          show-select
...
onMounted(() => {
  ...
  study
    .getSelectableStudies({ has_study_activity: true })
    .then((resp) => {
...
function getActivities(filters, options) {
  ...
  if (creationMode.value === 'createPlaceholder') {
    const params = {
      ...
      filters: `{"*":{"v":["${form.value.name}"]}, "status":{"v":["${statuses.FINAL}"]}}`,
    }
