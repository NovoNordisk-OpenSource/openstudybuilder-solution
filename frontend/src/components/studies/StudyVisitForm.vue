  ...
  (value) => {
    if (value) {
      epochs.getStudyVisit(selectedStudy.value.uid, value.uid).then((resp) => {
        ...
        if (!form.value.time_reference) {
          form.value.time_reference = {}
        }
        if (!form.value.visit_type) {
          form.value.visit_type = {}
        }
        if (!form.value.visit_contact_mode) {
          form.value.visit_contact_mode = {}
        }
        if (!form.value.epoch_allocation) {
          form.value.epoch_allocation = {}
        }
