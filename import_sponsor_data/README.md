...
with the dictionaries and sponsor data that OpenStudyBuilder requires.
...
These scripts populate the database through the API, so both Neo4j and the API
must be running before you start.

The full order is in the [root README](../README.md#populate-the-database):
initialise the database ([`db/`](../db/README.md)), import CDISC standards
([`import_standards/`](../import_standards/README.md)), start the API
([`api/`](../api/README.md)), then run the scripts here.
...
### Non-Standard Variables: `nonstandard_variables`
Imports Non-Standard Variables (NSV) — supplemental variables that extend
SDTM domains. Each row is created as an `ActivityItemClass` with a nested
`non_standard_variable` payload via `POST /activity-item-classes`. The CDISC
NSV registry is the primary source; sponsor-defined NSV CSVs can be appended
to `MDR_MIGRATION_NONSTANDARD_VARIABLES_FILES`.

| step | environment variable(s) |
|------|-------------------------|
| non-standard variables (one or more CSV files in the directory) | MDR_MIGRATION_NONSTANDARD_VARIABLES_DIRECTORY <br> MDR_MIGRATION_NONSTANDARD_VARIABLES_FILES <br> MDR_MIGRATION_NONSTANDARD_VARIABLES_LIBRARY <br> MDR_MIGRATION_NONSTANDARD_VARIABLES_AIC_FALLBACK <br> MDR_MIGRATION_NONSTANDARD_VARIABLES_DEFAULT_LENGTH_CHAR <br> MDR_MIGRATION_NONSTANDARD_VARIABLES_DEFAULT_LENGTH_NUM <br> MDR_MIGRATION_NONSTANDARD_VARIABLES_AUTO_APPROVE <br> MDR_MIGRATION_NONSTANDARD_VARIABLES_WRITE_LOGFILE |

Three source-file formats are recognised; the format is detected per-file
from the CSV header:

1. **CDISC** — comma-delimited, identified by a `variable_name` column. The
   CDISC NSV registry is distributed as an Excel file at
   https://www.cdisc.org/standards/terminology/non-standard-variables;
   convert it to the canonical CSV with:

   ```sh
   python cdisc_nsv_excel_converter.py path/to/cdisc_nsv.xls
   ```

   This writes `datafiles/sponsor_library/nonstandard_variables/cdisc_nsv.csv`.
2. **MMA** — pipe-delimited, identified by `cd_list_id` / `cd_val` columns.
   Sponsor-defined NSVs exported from the MMA system.
3. **OSB** — comma-delimited round-trip export produced by the OpenStudyBuilder
   UI's NSV listing, identified by the `non_standard_variable.code` header.
   Column names use the API's dotted paths. The file's `name` column is used
   verbatim as the slug; `status` / `version` / `start_date` / `uid` columns
   are ignored (a new UID is assigned on POST, and approval is governed by
   `MDR_MIGRATION_NONSTANDARD_VARIABLES_AUTO_APPROVE`). Because the export
   doesn't carry an `is_cdisc_defined` flag, OSB rows are always created as
   sponsor-defined.
...
This step imports data that is exported by the [export](../export/README.md) script.
...
- new_codelist_name: The name of the codelist in OpenStudyBuilder.
...
The `/import_sponsor_data/visit_type_excel_converter.py` script is designed to transform data from a Visit Types Excel file into a structured CSV format suitable for loading into OpenStudyBuilder (OSB). This script reads the specified Excel file, processes the data according to predefined rules, and generates a new CSV file formatted for OSB ingestion.
