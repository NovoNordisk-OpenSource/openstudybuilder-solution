...
class ApiBinding:
    ...
    async def get_all_from_api_async(
        self, path, session: aiohttp.ClientSession, params=None, items_only=True
    ):
        if params is None:
            params = {
                "page_number": 1,
        ...
        async with session.get(
            ...
            if response.ok:
                try:
                    ...
                    self.log.error(
                        f"Failed to decode json for '{path}', status: {status}, data: {textresult}"
                    )
                    return None
                if "items" in res and items_only:
                    self.metrics.icrement(path + "--GET", len(res["items"]))
            ...
            try:
                ...
                message = get_error_message(error_result)
            except aiohttp.ContentTypeError:
                message = await response.text()
            self.log.error(f"get '{path}', message: {message}, status: {status}")
            return None
    ...
    def get_all_identifiers(self, responses: list, identifier: str, value: str = None):
        ...
        for response_item in responses:
            if "." in identifier:
                ...
                ident = response_item
                for part in parts:
                    ident = ident.get(part, {})
            else:
                ident = response_item[identifier]
            identifiers[ident] = response_item[value]
        return identifiers
    ...
    async def new_version_patch_then_approve(
        ...
        session: aiohttp.ClientSession,
        approve: bool,
        continue_if_already_draft: bool = False,
    ):
        ...
        if not status_ok(status):
            ...
            if (
                "New draft version can be created only for FINAL versions" in error_msg
                and not continue_if_already_draft
            ):
                self.log.warning(
                    "Failed to create new version, item is already in DRAFT"
                )
            elif not continue_if_already_draft:
    ...
    async def find_term_by_submission_value_async(
        self, codelist_uid: str, submission_value: str, session: aiohttp.ClientSession
    ):
        filters = json.dumps(
            {"submission_value": {"v": [submission_value], "op": "eq"}}
        )
        result = await self.get_all_from_api_async(
    ...
    async def find_term_by_concept_id_async(
        self, concept_id: str, session: aiohttp.ClientSession
    ):
        filters = json.dumps({"concept_id": {"v": [concept_id], "op": "eq"}})
        result = await self.get_all_from_api_async(
            "/ct/terms/attributes",
            session,
            params={"filters": filters, "library": "CDISC"},
        )
        if result is not None and len(result) > 0:
            return result[0]
