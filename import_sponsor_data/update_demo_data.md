...
When creating the study, pick a study number that isn't already used in import_sponsor_data.
...
Then, check if import_sponsor_data already contains a study with the same uid as the new one.
...
To keep the files in import_sponsor_data manageable, we only copy over what is required.
